...
/* Homepage portfolio teleprompter */
(()=>{const rail=document.querySelector('#home-carousel');if(!rail)return;const prev=document.querySelector('.home-carousel-arrow.prev'),next=document.querySelector('.home-carousel-arrow.next');let paused=false,drag=false,startX=0,startScroll=0,moved=0,last=performance.now(),pos=rail.scrollLeft;const speed=48;const step=()=>rail.querySelector('.home-work-slide')?.getBoundingClientRect().width+18||420;prev?.addEventListener('click',()=>{rail.scrollBy({left:-step(),behavior:'smooth'});pos=rail.scrollLeft});next?.addEventListener('click',()=>{rail.scrollBy({left:step(),behavior:'smooth'});pos=rail.scrollLeft});rail.addEventListener('pointerdown',e=>{drag=true;paused=true;moved=0;startX=e.clientX;startScroll=rail.scrollLeft;rail.classList.add('dragging');rail.setPointerCapture?.(e.pointerId)});rail.addEventListener('pointermove',e=>{if(!drag)return;moved=Math.max(moved,Math.abs(e.clientX-startX));rail.scrollLeft=startScroll-(e.clientX-startX);pos=rail.scrollLeft});const end=()=>{drag=false;paused=false;rail.classList.remove('dragging');pos=rail.scrollLeft};rail.addEventListener('pointerup',end);rail.addEventListener('pointercancel',end);rail.addEventListener('mouseenter',()=>paused=true);rail.addEventListener('mouseleave',()=>{if(!drag)paused=false});rail.addEventListener('focusin',()=>paused=true);rail.addEventListener('focusout',()=>paused=false);function tick(now){const dt=Math.min((now-last)/1000,.05);last=now;if(!paused&&!document.hidden&&rail.scrollWidth>rail.clientWidth){pos+=speed*dt;const max=rail.scrollWidth-rail.clientWidth;if(pos>=max)pos=0;rail.scrollLeft=pos}requestAnimationFrame(tick)}requestAnimationFrame(tick)})();
